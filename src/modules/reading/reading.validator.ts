...
const readingSetIdSchema = z.string().trim().min(1).max(32);
...
export const publicReadingSetListQuerySchema = z.object({
  page: pageSchema,
  limit: limitSchema,
...
export const adminReadingSetListQuerySchema = z.object({
  page: pageSchema,
  limit: limitSchema,
  search: z.string().trim().optional(),
  level: levelSchema,
...
export const readingSetIdParamsSchema = z.object({
  id: readingSetIdSchema,
});

export const createReadingSetSchema = z.object({
  id: readingSetIdSchema,
  title: z.string().trim().min(1).max(255),
  passageHtml: z.string().trim().min(1).nullable().optional(),
  passageText: z.string().trim().min(1).nullable().optional(),
  level: bandScoreSchema,
  status: z.enum(["DRAFT", "PUBLISHED", "ARCHIVED"]).optional(),
  tagIds: z.array(uuidSchema).optional(),
});

export const updateReadingSetSchema = z
  .object({
...
export const readingQuestionIdParamsSchema = z.object({
  questionId: uuidSchema,
});

export const createReadingQuestionSchema = z.object({
